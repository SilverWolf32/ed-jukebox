//
// This code is based on code from ED-VOID, an exploration tracking and racing
// tool licensed under the MIT license.
// https://ed-void.com
// https://github.com/vovchisko/ed-void/
//
...
class Journal extends EE3 {
    ...
    ws_disconect() {
        if (this.ws && this.ws.readyState === WS.OPEN) {
            this.ws.close(1000, 'disconnect by client');
        }
    }
...
function _is_ignored_record(rec) {
	// ReceiveText and SendText are text chat
    if (['Music', 'ReceiveText'].includes(rec.event)) {
		return true;
	}
	// don't ignore /void commands
    if (rec.event === 'SendText' && !rec.Message.startsWith('/void')) {
		return true;
	}
    //todo: do we need to ignore something else?
	
	// TODO: include friends if racing "Friends Only" is on
	if (rec.event === "Friends" && false) {
		return false;
	}
	
	// ignore all unused events
	if (_used_records_list.includes(rec.event)) {
		return false;
	}
